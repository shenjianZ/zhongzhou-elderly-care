...
- 前端：`zzyl-vue3`，Vue 3 项目，使用 `pnpm` 构建，打包为 Nginx 镜像。
- 后端：`zzyl-admin`，Spring Boot 服务，JDK 11，容器内端口 `9000`。
- 中间件：MySQL 8、Redis 7，镜像使用阿里云镜像源。
- CI：GitHub Actions 构建并推送 `zzyl-admin`、`zzyl-vue3` 两个镜像。
...
3. 基于 `zzyl-vue3/Dockerfile` 使用 `pnpm install --frozen-lockfile && pnpm build:prod` 构建并推送 `zzyl-vue3:${tag}`。
...
registry.cn-hangzhou.aliyuncs.com/your-namespace/zzyl-vue3:latest
```
...
- `ZZYL_ADMIN_IMAGE`、`ZZYL_VUE3_IMAGE`
...
| `ZZYL_VUE3_IMAGE` | 前端镜像地址 | 是 | GitHub Actions 推送的 `zzyl-vue3` 镜像 |
...
| Nginx | 部署前端 `zzyl-vue3` 静态资源，并反向代理 `/prod-api/` 到后端 | 必须，已内置在前端镜像中 |
...
```bash
docker-compose -f docker-compose.full.yml --env-file .env pull
...
docker logs zzyl-vue3 --tail 100
curl -I http://127.0.0.1:${UI_PORT}/
```

常见问题：

- 前端打不开：检查 `zzyl-vue3` 容器、`UI_PORT`、安全组和防火墙。
